...
class WGAN_GP(object):
    ...
    def evaluate(self, D_model_path, G_model_path, tilt_angles, patch_size, image_size, save_path, original_image_size=None):
        ...
        for cnt in tqdm(range(tilt_angles)):
            ...
            # print("Image saved to 'tilt_noise_at_angle_{}.mrc'".format(str(cnt)))

        noise_complete_tilts = mrcfile.new(save_path + '/noise_tilts.mrc', overwrite=True)
        zero_tilts_np = np.array(zero_tilts)
        noise_complete_tilts.set_data(zero_tilts_np.astype(np.float32))
        print("Complete noise tilts saved to 'noise_tilts.mrc'")
