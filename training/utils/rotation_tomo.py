...
def rotate_vol_around_axis(vol, rot_angle, rot_axis, output_shape=None, order=3):
    """
    Rotates the 3D tensor 'vol' by 'rot_angle' degrees around 'rot_axis'.
    The rotated tensor, which is typically larger than the original one,
    is center-cropped such that it has dimensions 'output_shape'.
    If 'output_shape' is None, the rotated tensor is cropped to the dimensions of 'vol'.
    """
